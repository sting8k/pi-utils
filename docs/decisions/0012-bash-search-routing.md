...
5. **Wrapper unwrap (Amendment A1, 2026-09-14)**: owner's live smoke failed because pi-ctx-kit rewrites bash commands through `rtk` (`rg foo` → `rtk rg foo`), so the router never saw a search head. `matchBashSearch(command, unwrapPrefixes?)` strips **exactly one** leading token when it is in the whitelist — before the dangerous-char scan, itself unscanned — then applies rules 1–9 to the remainder. Whitelist: settings `bashRouter.unwrapPrefixes`, default `["rtk"]`; pre-amendment settings files get the section filled silently (no per-session warning toast). Never blind-stripping: `sudo grep …`, unknown wrappers (`foo rg x`), double prefixes (`rtk rtk rg x`), and non-search payloads (`rtk read X`) all stay null; unwrap never widens the matcher (`rtk grep -rn x .` stays null exactly like its unwrapped form). Default `[]` keeps the pure-function signature hermetic; the owner plans to revisit `"rtk"` as default before npm publish.
...
- Amendment A1: unwrap matrix (prefixed routing, sudo/double/unknown/`rtk read`/empty-list nulls) + bashRouter settings tests (silent default fill, custom/empty whitelist, invalid warn). Live smoke re-run by owner after push (needs `pi /reload`).
